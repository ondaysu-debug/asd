...
DEXES_BY_CHAIN = {
    ...
    "base":     ["aerodrome", "uniswap-v3", "uniswap-v2", "sushiswap", "pancakeswap-v3"],
    "ethereum": ["uniswap", "uniswap-v2", "uniswap-v3", "sushiswap", "pancakeswap-v3"],
}
...
def is_base_ok(base_token: dict, chain: str) -> bool:
    ...
    if addr and addr in NATIVE_ADDR.get(chain, set()): return False
...
def _make_buckets():
    ...
    if not USE_TWO_CHAR_BUCKETS:
        return one[:MAX_BUCKETS_PER_CHAIN]
    two = [a+b for a in BUCKET_ALPHABET for b in BUCKET_ALPHABET]
    buckets = one + two
    return buckets[:MAX_BUCKETS_PER_CHAIN]
...
def ds_search_native_pairs(chain: str):
    ...
    for p in all_pairs:
        if (p.get("chainId") or "").lower() != chain: continue
        ...
        if qAddr == native_cmp and bAddr != native_cmp:
            ...
            skipped_not_native += 1; continue

        if not is_base_ok(norm_baseTok, chain):
            skipped_maj += 1; continue

        fdv = p.get("fdv") or p.get("marketCap") or 0
        try: fdv = float(fdv or 0)
        except: fdv = 0.0

        txns24 = (p.get("txns") or {}).get("h24", {}) or {}
        tx24h = int((txns24.get("buys") or 0) + (txns24.get("sells") or 0))

        if not (MARKET_CAP_MIN <= fdv <= MARKET_CAP_MAX): continue
        if tx24h > TX24H_MAX: continue

        vol = p.get("volume") or {}
        vol5m  = float((vol.get("m5")  or 0) or 0)
        vol24h = float((vol.get("h24") or 0) or 0)
...
def fetch_gecko_for_pool(chain, pool_addr):
    ...
    try:
        ...
        txm5    = (a.get("transactions") or {}).get("m5", {}) or {}
        tx_5m   = int((txm5.get("buys") or 0) + (txm5.get("sells") or 0))
        return vol_5m, tx_5m, vol_24h
    except Exception:
        return 0.0, 0, 0.0
...
def maybe_alert(conn, meta):
    ...
    vol5m, tx5m, vol24h = fetch_gecko_for_pool(chain, pool)
    if (vol5m, tx5m, vol24h) == (0.0, 0, 0.0):
        vol5m  = float(meta.get("vol5m_ds", 0.0))
        vol24h = float(meta.get("vol24h_ds", 0.0))
        tx5m   = int(meta.get("tx5m_ds", 0))

    prev24 = max(vol24h - vol5m, 0.0)
    if vol5m <= prev24:
        return  # нет алерта

    set_last_alert_ts(conn, pool, int(now_utc().timestamp()))
    ratio = (vol5m / prev24) if prev24 > 0 else float('inf')
    ...
    text = (
        ...
        f"5m Vol: ${nice(vol5m)}\n"
        f"Prev 24h Vol (excl. current 5m): ${nice(prev24)}\n"
        f"Ratio 5m/prev24h: {ratio:.2f}x\n"
        f"Link: {meta.get('url')}"
    )
    tg_send(text)
    print("Alert:", chain, pool, "vol5m >", "prev24h")
